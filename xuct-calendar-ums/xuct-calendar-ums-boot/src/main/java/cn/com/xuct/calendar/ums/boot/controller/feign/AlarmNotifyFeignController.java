...
public class AlarmNotifyFeignController {
    ...
    @PostMapping("")
    public R<String> notify(@RequestBody AlarmNotifyFeignInfo alarmNotifyFeignInfo) {
        Member member = memberService.getById(alarmNotifyFeignInfo.getCreateMemberId());
        /*1. 站内信 直接发送 */
        if (alarmNotifyFeignInfo.getType() == 1) {
            AlarmNotifyEvent alarmNotifyEvent = new AlarmNotifyEvent(this, alarmNotifyFeignInfo.getSummary(),
                    alarmNotifyFeignInfo.getStartDate(), alarmNotifyFeignInfo.getComponentId(),
                    alarmNotifyFeignInfo.getCreateMemberId(), member.getName(), alarmNotifyFeignInfo.getIds()
            );
            SpringContextHolder.publishEvent(alarmNotifyEvent);
            return R.status(true);
        }
        /*2. 邮箱或openid 查询*/
        List<Column> columns = Lists.newArrayList(Column.of("member_id", alarmNotifyFeignInfo.getIds(), ColumnEnum.in));
        if (alarmNotifyFeignInfo.getType() == 2) {
            columns.add(Column.of("identity_type", IdentityTypeEnum.email));
        } else if (alarmNotifyFeignInfo.getType() == 3) {
        ...
        if (alarmNotifyFeignInfo.getType() == 2) {
            ...
            basicServicesFeignClient.sendEmail(EmailFeignInfo.builder().subject("事件提醒").tos(tos).template("alarm").params(new HashMap<>() {{
                put("summary", alarmNotifyFeignInfo.getSummary());
                put("start_date", alarmNotifyFeignInfo.getStartDate());
                put("create_member_name", member.getName());
                put("create_member_id", alarmNotifyFeignInfo.getCreateMemberId());
            }}).build());
            return R.status(true);
        }
        /*3.小程序 发送订阅消息 */
