...
@RequestMapping("/api/inner/v1/sms")
public class SmsController {
    ...
    @PostMapping("")
    public R<String> smsCode(@Validated @RequestBody SmsCodeDto smsCodeDto) {
        Assert.isTrue(!CollectionUtils.isEmpty(smsCodeDto.getPhones()), "联系人为空");
        String[] phones = new String[smsCodeDto.getPhones().size()];
        for (int i = 0; i < smsCodeDto.getPhones().size(); i++) {
            phones[i] = "+86".concat(smsCodeDto.getPhones().get(i));
        }
        try {
            tencentSmsClient.sendSmsCode(tencentProperties.getSms().getTemplateId().get(smsCodeDto.getTemplate()), phones, smsCodeDto.getCode());
